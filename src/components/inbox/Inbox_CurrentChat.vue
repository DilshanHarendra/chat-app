<template>
  <div class="relative flex flex-col flex-1">
    <div v-if="userProfile" class="z-20 flex flex-grow-0 flex-shrink-0 w-full pr-3 bg-white border-b">
      <div @click="$emit('show-profile')" class="w-12 h-12 mx-4 my-2 bg-blue-500 bg-center bg-no-repeat bg-cover rounded-full cursor-pointer"
           :style="{backgroundImage: `url(${userProfile.picture})`}">
      </div>
      <div @click="$emit('show-profile')" class="flex flex-col justify-center flex-1 overflow-hidden cursor-pointer">
        <div class="overflow-hidden text-base font-medium leading-tight text-gray-600 whitespace-no-wrap">{{userProfile.name}}</div>
    ...
    <Inbox_CurrentChat_MessageBox v-if="userProfile"/>
...
<script>
import Inbox_CurrentChat_MessageBox from "@/components/inbox/Inbox_CurrentChat_MessageBox";
export default {
  name: "Inbox_CurrentChat",
  components: {Inbox_CurrentChat_MessageBox},
  created() {
  ...
  computed:{
    ...
    userProfile(){
      return this.$store.getters['users/id'](this.userId)
    },
